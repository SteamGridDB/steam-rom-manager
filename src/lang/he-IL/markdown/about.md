## מידע מערכת

במקרה שאתם מתקשים עם SRM, אתם מוזמנים לבקר ב-[דיסקורד](https://discord.gg/bnSVJrz) או ב-[סוברדדיט](https://www.reddit.com/r/SteamRomManager/) ולבקש את עזרתנו. כדי שנוכל לעזור לך, נשמח שתספק לנו את המידע הבא:

* גרסת SRM: **#{APP[גרסה]}** emudeckText
* מערכת הפעלה: **#{APP[os]}**

## קישורים שימושיים

* [שחרורי SRM](https://github.com/SteamGridDB/steam-rom-manager/releases)
* [שרת הדיסקורד](https://discord.gg/bnSVJrz)
* [r/SteamRomManager](https://www.reddit.com/r/SteamRomManager/)
* [קבוצת סטים](https://steamcommunity.com/groups/steamrommanager)
* [מאגר גיטהאב](https://github.com/SteamGridDB/steam-rom-manager)
* [➡פרויקט התרגומים של Crowdin](https://crowdin.com/project/steam-rom-manager)

## תמיכה

עזרו לתמוך בהתקדמות מתמשכת ב-Steam ROM Manager בכתובת:

<a href="https://www.buymeacoffee.com/cbartondock">
  <img src="https://raw.githubusercontent.com/SteamGridDB/steam-rom-manager/master/src/assets/images/buy-me-a-coffee.png" alt="קנה לי קפה" width="160">
</a>

אם אתם נהנים מ-Steam ROM Manager ורוצים שהוא ימשיך להיות שימושי, שקלו גם לתמוך ב-Patreon של [SteamGridDB](https://www.steamgriddb.com/). [SteamGridDB](https://www.steamgriddb.com/) מאחסן את כל יצירות האמנות ש-Steam ROM Manager משתמש בהן כדי להפוך את ספריית ה-Steam שלכם לקנאת העיר, אז כנראה שכדאי שנעזור להם לשמור על האורות דולקים.

<a href="https://www.patreon.com/steamgriddb">
    <img src="https://c5.patreon.com/external/logo/become_a_patron_button@2x.png" width="160">
</a>

## תרומות
* `FrogTheFrog`{.noWrap} - יוצר SRM, כבר לא פעיל ב-SRM אך עובד כעת על [תוסף MoonDeck](https://github.com/FrogTheFrog/moondeck)

### מתחזקים נוכחיים
* `lontanadascienza`{.noWrap} - עדכון SRM לטיפול בגיבורים, פוסטרים, לוגואים ואייקונים. נוסף מנהל חריגים, ניהול בקרים, מנתחי פלטפורמה (Epic, GOG, Steam וכו'), בין היתר. מתקן באגים כמעט באותה מהירות שהוא מכניס אותם.
* `Zennn`{.noWrap} - נוספה תמיכה בקטגוריות, לוקליזציה של שפות ושרת דיווחי באגים בין היתר. Big boss over at [SteamGridDB](https://www.steamgriddb.com/).
* `KenCinder`{.noWrap} - Manages and creates community parser presets and helps user setup SRM in [Discord](https://discord.gg/bnSVJrz).

### תרומות לתכונות
* `dragoonDorise`{.noWrap} - נוספו ערכות נושא חדשות ל-SRM. יוצר ומתחזק נוכחי של [EmuDeck](https://www.emudeck.com/).
* `alvaromunoz`{.noWrap} - מנתח Game Pass נוסף ל-SRM.
* `lexarvn`{.noWrap} - Added the Amazon Games parser and the itch.io parser.
* `CarJem`{.noWrap} - Added the manual parser.
* `MattMckenzy`{.noWrap} - Added ability to import and export image choices.
* `Apalatn`{.noWrap} - Added an install drive redirect option to the itch.io parser.
* `OneMoreByte`{.noWrap} - Made itch.io parser work on linux and mac.
* `UndarkAido`{.noWrap} - Added shortcut passthrough for Linux's .desktop shortcuts.
* `HazardousBackup`{.noWrap} - Added option to GOG parser to parse Registry instead of GOG's DB.

### קהילה
* `HE Spoke`{.noWrap} - created a community around SRM. Creator of [Steam](https://steamcommunity.com/groups/steamrommanager) and [Discord](https://discord.gg/bnSVJrz) groups. עוזר גם למשתמשים להגדיר SRM ב-[דיסקורד](https://discord.gg/bnSVJrz).
* `livedeht`{.noWrap} - Helps users setup SRM with and without [EmuDeck](https://www.emudeck.com/). Current maintainer of [EmuDeck](https://www.emudeck.com/).
* `Choko`{.noWrap} - Helps users setup SRM in [Discord](https://discord.gg/bnSVJrz).
* `Xav83`{.noWrap} - Creator and maintainer of SRM's [Chocolatey package](https://community.chocolatey.org/packages/steam-rom-manager)
