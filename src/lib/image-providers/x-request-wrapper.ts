...
export class xRequestWrapper<T extends GenericProvider> extends xRequest {
  ...
  private canRetry(status: number, config: any) {
    let retryCount = this.specialErrors[`${status}`] ? this.specialErrors[`${status}`].retryCount || this.retryCount : this.retryCount;
    ...
    if (retryCount > 0) {
      if (config['retryCount'] === undefined) {
        ...
        return (config['retryCount']++ < retryCount);
    }
    else
      return false;
  }
      ...
      return promise.catch((data: xRequestError) => {
        if (data.error) {
          if (data.error.status === 429) {
            ...
            return this.errorHandler(this.makeRequest(data.config, timeout * 1000));
          }
  ...
  setSpecialErrors(specialErrors: { [statusCode: string]: { retryCount?: number, silent: boolean } }) {
  ...
  get(url: string, params?: { [parameter: string]: string }, responseType: XMLHttpRequestResponseType = 'json', delay: number = 0) {
    return this.errorHandler(super.request(url, {
      method: 'GET',
  ...
  logError(value: any, url?: string) {
    if (value.error)
      ...
      this.proxy.error(typeof value === 'object' ? value.status || JSON.stringify(value) : value, url);
  }
}
