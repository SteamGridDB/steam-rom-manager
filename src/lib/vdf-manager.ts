...
import { json, steam } from "./helpers";
...
export class VdfManager {
    ...
    private readVDFListData(steamDirectory: string, userId: string) {
        return json.read<VDFListFileData[]>(path.join(path.dirname(this.listData[steamDirectory][userId].shortcuts.path), paths.savedListFilename), []);
    }

    private writeVDFListData(steamDirectory: string, userId: string, data: VDFListFileData[]) {
        return json.write(path.join(path.dirname(this.listData[steamDirectory][userId].shortcuts.path), paths.savedListFilename), data);
    }
            ...
            let getEntry = function (shortcutsData: any[], appID: string) {
                for (let i = 0; i < shortcutsData.length; i++) {
                    if (shortcutsData[i]) {
                        ...
                        if (appID === steam.generateAppId(exe, appName))
                            return i;
                    }
                }
                return -1;
        ...
        for (let steamDirectory in this.listData) {
            ...
            for (let userId in this.listData[steamDirectory]) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (dataObject.tree[steamDirectory][userId][steam.generateAppId(exe, appName)] === undefined) {
                        dataObject.numberOfUsers++;
                        dataObject.tree[steamDirectory][userId][steam.generateAppId(exe, appName)] = data[i];
                    }
                }
            }
        }
