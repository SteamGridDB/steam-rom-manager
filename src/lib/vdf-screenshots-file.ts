...
export class VDF_ScreenshotsFile {
  ...
  read() {
    return fs
      ...
      .then((data) => {
        ...
        if (this.fileData[this.topKey] === undefined) this.fileData[this.topKey] = {};
        if (this.fileData[this.topKey]["shortcutnames"] === undefined) this.fileData[this.topKey]["shortcutnames"] = {};
  ...
  private removeExtraneous(exAppId: string): Promise<VDF_Error | void> {
    return glob(`${exAppId}.*`, { dot: true, cwd: this.gridDirectory, absolute: true }).then((files: string[]) => {
      let errors: Error[] = [];
  ...
  async write(batch: boolean, batchSizeInput?: number) {
      ...
      for (let j = b * batchSize; j < Math.min((b + 1) * batchSize, addableAppIds.length); j++) {
        ...
        if (!nintendoSucks) {
          ...
          batchAddPromises.push(
            imageDownloader
              .downloadAndSaveImage(data.url, gridPath, 4)
              .then(() => {
                if (/^\d+$/.test(appId)) {
                  ...
                  let symlinkType: any = "file";

                  if (os.type() == "Windows_NT") {
                    symlinkType = "junction";
                  }

                  fs.symlinkSync(gridPath, symPath, symlinkType);
                }
                return gridPath;
              })
    ...
    return Promise.all(extraneousPromises)
