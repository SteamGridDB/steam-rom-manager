...
export class DraggableDirective implements OnInit, OnDestroy, OnChanges {
    ...
    private pos: {
        parent: { left: number, top: number, width: number, height: number, selfOffsetLeft: number, selfOffsetTop: number },
        self: { left: number, top: number, width: number, height: number, mouseOffsetLeft: number, mouseOffsetTop: number }
    };

    @Input('ng-draggable') private userOptions: userOptionsType;
    @ContentChildren(DraggableDirective) childrenDirectives: QueryList<DraggableDirective>;
    @ContentChildren('dragArea', { read: ElementRef }) childrenDragAreas: QueryList<ElementRef>;
    ...
    private onMousedown(event: MouseEvent) {
        if (this.enabled) {
            ...
            if (this.canBeDragged(event.target))
    ...
    private onMousemove(event: DragEvent) {
        if (this.dragging && this.elementRef.nativeElement) {
            ...
            el.style.setProperty('left', this.calculateX(event.clientX) + 'px', 'important');
            el.style.setProperty('top', this.calculateY(event.clientY) + 'px', 'important');
            this.redrawChildren();
        }
    }
    ...
    private calculateX(mouseX: number) {
        let el = (<HTMLElement>this.elementRef.nativeElement);
        let newLeft = mouseX - this.pos.parent.left - this.pos.parent.selfOffsetLeft - this.pos.self.mouseOffsetLeft;
        switch (this.dragLimit) {
            case 'parent':
                return this.fitToLimits(0 - this.pos.parent.selfOffsetLeft, newLeft, this.pos.parent.width - this.pos.self.width - this.pos.parent.selfOffsetLeft);
            case 'viewport':
                return this.fitToLimits(0 - this.pos.parent.left - this.pos.parent.selfOffsetLeft, newLeft, window.innerWidth - this.pos.self.width - this.pos.parent.left - this.pos.parent.selfOffsetLeft);
            default:
                return newLeft;
        }
    }
    ...
    private calculateY(mouseY: number) {
        let el = (<HTMLElement>this.elementRef.nativeElement);
        let newTop = mouseY - this.pos.parent.top - this.pos.parent.selfOffsetTop - this.pos.self.mouseOffsetTop;
        switch (this.dragLimit) {
            case 'parent':
                return this.fitToLimits(0 - this.pos.parent.selfOffsetTop, newTop, this.pos.parent.height - this.pos.self.height - this.pos.parent.selfOffsetTop);
            case 'viewport':
                return this.fitToLimits(0 - this.pos.parent.top - this.pos.parent.selfOffsetTop, newTop, window.innerHeight - this.pos.self.height - this.pos.parent.top - this.pos.parent.selfOffsetTop);
            default:
                return newTop;
        }
    }

    private updatePosData(onMouseDown: boolean, mouseX?: number, mouseY?: number, ) {
        let el = (<HTMLElement>this.elementRef.nativeElement);
        ...
        if (onMouseDown) {
            ...
            if (mouseX !== undefined)
                ...
                this.pos.self.mouseOffsetLeft = 0;

            if (mouseY !== undefined)
                this.pos.self.mouseOffsetTop = mouseY - rect.top;
            else
                this.pos.self.mouseOffsetTop = 0;
        }
        ...
        if (el.parentElement) {
            ...
            if (onMouseDown) {
                this.pos.parent.selfOffsetLeft = rect.left - parentRect.left - parseFloat(window.getComputedStyle(el, null).getPropertyValue('left'));
                this.pos.parent.selfOffsetTop = rect.top - parentRect.top - parseFloat(window.getComputedStyle(el, null).getPropertyValue('top'));
            }
    ...
    private saveUserSelect() {
        if (this.userSelect === undefined && this.elementRef.nativeElement) {
            this.userSelect = (<HTMLElement>this.elementRef.nativeElement).style.userSelect;
    ...
    private restoreUserSelect() {
        if (this.userSelect !== undefined && this.elementRef.nativeElement) {
            (<HTMLElement>this.elementRef.nativeElement).style.userSelect = this.userSelect;
            this.userSelect = undefined;
        }
    }
    ...
    private savePosition() {
        if (this.positionProperties === undefined && this.elementRef.nativeElement) {
            let el = (<HTMLElement>this.elementRef.nativeElement);
            this.positionProperties = {
                left: { value: el.style.getPropertyValue('left'), priority: el.style.getPropertyPriority('left') },
                top: { value: el.style.getPropertyValue('top'), priority: el.style.getPropertyPriority('top') },
                right: { value: el.style.getPropertyValue('right'), priority: el.style.getPropertyPriority('right') },
                bottom: { value: el.style.getPropertyValue('bottom'), priority: el.style.getPropertyPriority('bottom') }
            };
    ...
    private parseDragAreas(directiveChildren: QueryList<DraggableDirective>, areaChildren: QueryList<ElementRef>) {
            ...
            areaChildren.toArray().forEach((child) => {
                ...
                if (childEl !== null && childEl !== el) {
                    ...
                    do {
                        ...
                        if (directiveArray.filter(item => item.elementRef.nativeElement === parentEl).length) {
                            if (parentEl === el)
                                dragAreas.push(child);
                            else
                                break;
                        }
                    } while (parentEl);
    ...
    private recalculate() {
        if (!this.dragging && this.elementRef.nativeElement) {
            ...
            let el = (<HTMLElement>this.elementRef.nativeElement);
            el.style.setProperty('left', this.calculateX(this.pos.self.left) + 'px', 'important');
            el.style.setProperty('top', this.calculateY(this.pos.self.top) + 'px', 'important');
        }
    }
    ...
    private canBeDragged(target: EventTarget) {
        if (target === this.elementRef.nativeElement)
            ...
            for (let i = 0; i < this.dragAreas.length; i++) {
                if (target === this.dragAreas[i].nativeElement)
                    return true;
            }
            return false;
        }
    ...
    resetPosition() {
        if (this.positionProperties !== undefined && this.elementRef.nativeElement) {
            ...
            if (this.positionProperties.left.value)
                el.style.setProperty('left', this.positionProperties.left.value, this.positionProperties.left.priority);
            else
                el.style.removeProperty('left');

            if (this.positionProperties.top.value)
                el.style.setProperty('top', this.positionProperties.top.value, this.positionProperties.top.priority);
            else
                el.style.removeProperty('top');

            if (this.positionProperties.right.value)
                el.style.setProperty('right', this.positionProperties.right.value, this.positionProperties.right.priority);
            else
                el.style.removeProperty('right');

            if (this.positionProperties.bottom.value)
                el.style.setProperty('bottom', this.positionProperties.bottom.value, this.positionProperties.bottom.priority);
            else
    ...
    ngOnInit() {
        if (this.elementRef.nativeElement) {
            this.position = window.getComputedStyle(this.elementRef.nativeElement, null).getPropertyValue('position');
            if (this.position === 'static') {
                throw Error('Only "relative", "absolute" and "fixed" positions are supported');
            }
        }
        else
            throw Error('Unable to access DOM on init (draggable)');
    }
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (changes['userOptions']) {
            ...
            if (value.dragLimit !== undefined && this.validateDragLimit(value.dragLimit)) this.dragLimit = value.dragLimit;
