...
export class LoggerService {
  ...
  submitReport(description: string, useVDFs: boolean, discordUser?: string, steamDirectory?: string) {
    return new Promise<{key: string, deleteKey: string}>((resolve, reject)=>{
      ...
      if(fs.existsSync(paths.userConfigurations)) {
        let userConfigurations = new Blob([fs.readFileSync(paths.userConfigurations)]);
        body.append('files[]', userConfigurations, "userConfigurations.json");
      }
      if(fs.existsSync(paths.userExceptions)) {
        let userExceptions = new Blob([fs.readFileSync(paths.userExceptions)]);
      ...
      if(useVDFs) {
        ...
        let userDirs = fs.readdirSync(userData, { withFileTypes: true }).filter(dirent=>dirent.isDirectory() && dirx.test(dirent.name)).map(dirent=>dirent.name);
        for(let userDir of userDirs) {
          let shortcutsVDFPath = path.join(steamDirectory,'userdata',userDir,'config','shortcuts.vdf');
          ...
          let screenshotsVDFPath = path.join(steamDirectory,'userdata',userDir,'760','screenshots.vdf');
          if(fs.existsSync(screenshotsVDFPath)) {
            let screenshotsVDF = new Blob([fs.readFileSync(screenshotsVDFPath)]);
            body.append('files[]', screenshotsVDF, `screenshots_${userDir}.vdf`);
          }
        }
      }
      return LoggerService.xRequest.request('https://logs.jozen.blue/logs', {
        responseType: 'json',
        method: 'POST',
        body: body,
        timeout: 1000
