...
export class PreviewService {
  ...
  saveData({ batchWrite, removeAll }: { batchWrite: boolean; removeAll: boolean; pegasus: boolean }): Promise<any> {
    let knownSteamDirectories = this.parsersService.getKnownSteamDirectories();
    if (this.previewVariables.listIsBeingSaved) {
      return Promise.resolve().then(() => {
        this.loggerService.info(this.lang.info.listIsBeingSaved, { invokeAlert: true, alertTimeout: 3000 });
        return false;
      });
    } else if (!removeAll && this.previewVariables.numberOfListItems === 0) {
      return Promise.resolve().then(() => {
        this.loggerService.info(this.lang.info.listIsEmpty, { invokeAlert: true, alertTimeout: 3000 });
        return false;
      });
    } else if (this.previewVariables.listIsBeingRemoved) {
      return Promise.resolve().then(() => {
        this.loggerService.info(this.lang.info.listIsBeingRemoved, { invokeAlert: true, alertTimeout: 3000 });
        return false;
      });
      ...
      .then(() => {
        if (removeAll) {
          ...
          this.loggerService.info(
            this.lang.info.writingVDF_entries__i.interpolate({ batchSize: this.appSettings.batchDownloadSize }),
  ...
  getAllParsers() {
    return this.previewData
      ? Object.entries(this.previewData)
          .map((dir) =>
            Object.entries(dir[1])
              .map((user) => Object.entries(user[1].apps).map((app) => app[1].configurationTitle))
              .reduce(this.union, [])
          )
          .reduce(this.union, [])
      : [];
      ...
      this.parsersService
        ...
        .then((data) => {
          ...
          if (data.parsedData.parsedConfigs.length > 0) {
            ...
            else this.loggerService.info(this.lang.info.parserFoundNoFiles, { invokeAlert: true, alertTimeout: 3000 });
          }
        })
        .then((previewData) => {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (this.appSettings.previewSettings.retrieveCurrentSteamImages)
          return steam.getGridImagesForTree(steamTreeData).then((gridData) => {
            return { gridData, steamTreeData };
          });
        else return { gridData: steamTreeData, steamTreeData };
      })
      .then((treeData) => {
        return steam.getNonSteamShortcutsData(treeData.steamTreeData).then((shortcutData) => {
      ...
      .then((treeData) => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (previewData[config.steamDirectory] === undefined) previewData[config.steamDirectory] = {};
            ...
            for (let k = 0; k < data[i].files.length; k++) {
                ...
                if (shortcutsData[config.steamDirectory][userAccount.accountID][appID]["icon"] !== undefined) {
                  if (
                    file.localImages["icon"].indexOf(
                      shortcutsData[config.steamDirectory][userAccount.accountID][appID]["icon"]
                    ) === -1
                  ) {
              ...
              for (const artworkType of artworkTypes) {
                if (this.appImages[artworkType][file.imagePool] === undefined) {
                  ...
                  this.appImages[artworkType][file.imagePool].searchQueries = _.union(
                    currentQueries,
                    file.onlineImageQueries
                  );
                  this.appImages[artworkType][file.imagePool].defaultImageProviders = _.union(
                    currentProviders,
              ...
              if (previewData[config.steamDirectory][userAccount.accountID].apps[appID] === undefined) {
                ...
                let currentCategories =
                  previewData[config.steamDirectory][userAccount.accountID].apps[appID].steamCategories;
                previewData[config.steamDirectory][userAccount.accountID].apps[appID].steamCategories = _.union(
                  currentCategories,
  ...
  downloadImageUrls(imageType: string, imageKeys?: string[], imageProviders?: string[]) {
    if (!this.appSettings.offlineMode) {
      ...
      for (let i = 0; i < imageKeys.length; i++) {
        ...
        let imageProvidersForKey: string[] =
          imageProviders === undefined || imageProviders.length === 0 ? image.defaultImageProviders : imageProviders;
          ...
          if (numberOfQueriesForImageKey > 0) {
            ...
            for (let j = 0; j < image.searchQueries.length; j++) {
              this.imageProviderService.instance.retrieveUrls(
                ...
                imageProvidersForKey,
                <K extends keyof ProviderCallbackEventMap>(event: K, data: ProviderCallbackEventMap[K]) => {
                  switch (event) {
                    case "error":
                      {
                    ...
                    case "image":
                      imageQueue.push(null, () => {
                        let newImage = this.addUniqueImage(
                          imageKeys[i],
                          (data as ProviderCallbackEventMap["image"]).content,
                          imageType
                        );
                        if (newImage !== null && this.appSettings.previewSettings.preload) this.preloadImage(newImage);
            ...
            for (const appId in this.previewData[directory][userId].apps) {
              ...
              for (const artworkType of artworkTypes) {
                if (appImage.getCurrentImage(app.images[artworkType], this.appImages[artworkType])) {
                  const imageUrl = appImage.getCurrentImage(
